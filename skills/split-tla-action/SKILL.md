...
If the specification lacks a `pc` variable, first use the **"add-tla-variable"** skill to add `pc` variable. If the actions are thread- or process-specific, `pc` variable must be a function of the thread or process identifier to a string constant. For example,
